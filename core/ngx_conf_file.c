...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    for ( ;; ) {
        rc = ngx_conf_read_token(cf);   /* 读入一个token，一般是一行 */
         ...
         * 判断cf是否有handler回调，如果有的话，优先调用handler回调，
         * 如果没有，则会进入ngx_conf_handler进行一般处理
         */
        if (cf->handler) {
...
ngx_conf_handler(ngx_conf_t *cf, ngx_int_t last)
{
    ...
    found = 0;
    /* 遍历每个模块，检查token（就是上面的name，
     * 比如这里的name可能是worker_process）是在哪个模块中被定义，并且进行处理
     */
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    for ( ;; ) {
        ...
        if (b->pos >= b->last) {    /* 读取的内容分析完了，重新读取 */

            if (cf->conf_file->file.offset >= file_size) {  /* 整个文件分析完成 */
        ...
        if (last_space) {   /* 前面一个元素分析完了，开始分析下一个元素 */
            if (ch == ' ' || ch == '\t' || ch == CR || ch == LF) {  /* 滤过空格，制表符，回车，换行 */
                continue;
            }
            ...
            case '"':   /* 以双引号开始，开始元素位置加一 */
                start++;
                d_quoted = 1;
                last_space = 0;
                continue;

            case '\'':  /* 以单引号开始，开始元素位置加一 */
                start++;
                s_quoted = 1;
                last_space = 0;
                continue;
...
ngx_conf_include(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_open_glob(&gl) != NGX_OK) { /* 调用glob函数将查找所有匹配模式的文件名 */
        ngx_conf_log_error(NGX_LOG_EMERG, cf, ngx_errno,
