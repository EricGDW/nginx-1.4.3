...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    tp = ngx_timeofday();   /* 取出之前的ngx_cache_time */
    tp->sec = 0;
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        module = ngx_modules[i]->ctx;   /* 得到core modules */

        /* 对每个core模块调用模块内部的钩子ngx_xxx_module_create_conf，当然第一个模块是core */
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        if (module->init_conf) {    /* 配置文件解析完毕后，就初始化core module的config */
