...
typedef struct {
    ...
    unsigned                 test_dir:1;            /* 是否为测试目录 */
    unsigned                 test_only:1;           /* 是否只用于测试 */
    unsigned                 log:1;                 /* 是否记录log */
    ...
    unsigned                 is_directio:1;         /* 是否为directio */
...
struct ngx_cached_open_file_s {
    ngx_rbtree_node_t        node;      /* cache文件所在红黑树节点，红黑树是为了方便查找(需要根据文件名迅速得到fd)*/
    ngx_queue_t              queue;     /* cache文件所在队列，队列为了方便超时管理 */
    ...
    ngx_fd_t                 fd;        /* 文件描述符fd */
    ngx_file_uniq_t          uniq;      /* i节点编号，文件属性中的st_ino，用于判断文件是否被修改 */
    time_t                   mtime;     /* 最后修改时间 */
    off_t                    size;      /* 文件大小 */
    ...
    ngx_event_t             *event;         /* 文件事件链表 */
};

/* 文件缓存表；管理所有缓存文件的结构 */

typedef struct {
    ngx_rbtree_t             rbtree;        /* 文件缓存表的红黑树表示，一个cache文件对应红黑树的一个节点 */
    ngx_rbtree_node_t        sentinel;      /* 哨兵 */
    ngx_queue_t              expire_queue;  /* 超时的缓存文件队列 */

    ngx_uint_t               current;       /* 当前cache的文件个数；初始值为0 */
    ngx_uint_t               max;           /* 最大cache文件数，指令 "open_file_cache"的max参数指定的cache文件个数 (ngx_open_file_cache_init()函数中设置) */
    time_t                   inactive;      /* 超时时间，指令 "open_file_cache"的inactive参数指定的时间 (ngx_open_file_cache_init()函数中设置) */
} ngx_open_file_cache_t;


typedef struct {
    ngx_open_file_cache_t   *cache;     /* 文件缓存表 */
    ngx_cached_open_file_t  *file;      /* 对应的cache文件 */
    ngx_uint_t               min_uses;  /* 指定cache最小使用的次数，默认是1次 */
