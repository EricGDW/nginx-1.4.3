...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    if (ngx_use_accept_mutex) {
        ...
        if (ngx_accept_disabled > 0) {
             ...
             * 因此，这就保证了只有一个进程拥有监听套接口，故所有进程阻塞在epoll_wait时，才不会惊群现象。
             */
            ...
            if (ngx_accept_mutex_held) {
                /*
                 *　如果进程获得了锁，将添加一个NGX_POST_EVENTS标志。
    ...
    (void) ngx_process_events(cycle, timer, flags); /* 开始wait事件 */
    ...
    */
    ...
    if (ngx_accept_mutex_held) {    /* 所有accept事件处理完之后，如果持有锁的话，就释放掉。 */
...
ngx_event_module_init(ngx_cycle_t *cycle)
{
    ...
    /*如果关闭了master进程，就不需要下面的步骤了。因为关闭了master进程就是
    ...
    /*创建accept互斥体。  accept互斥体的实现依赖是否支持原子操作，如果有相应的原子操作；
    就是用取得的这段共享内存来实现accept互斥体；否则，将使用文件 锁来实现accept互斥体。
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        rev->handler = ngx_event_accept;    /*注册监听套接口读事件的回调函数ngx_event_accept*/
...
ngx_events_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(void **) conf = ctx;
    ...
    for (i = 0; ngx_modules[i]; i++) {  /* 循环调用每个事件模块的create_conf函数，创建配置结构*/
        if (ngx_modules[i]->type != NGX_EVENT_MODULE) {
            continue;
        }
